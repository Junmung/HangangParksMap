...
public class ObjectBuilder {
    ...
    private void appendCone(Geometry.Cone cone, int numPoints) {
        final int startVertex = offset / (FLOATS_PER_VERTEX+1);
        final int numVertices = sizeOfCircleInVertices(numPoints);
    ...
    public static GeneratedData createArrow(Geometry.Cylinder puck, Geometry.Cone top, int numPoints) {
        int size = (3*sizeOfCircleInVertices(numPoints) + sizeOfOpenCylinderInVertices(numPoints));
        ObjectBuilder builder = new ObjectBuilder(size);
        builder.appendCone(top, numPoints);
        builder.appendOpenCylinder(puck, numPoints);
        Geometry.Circle cylinderBottom = new Geometry.Circle(puck.center.translateY(-puck.height/2f), puck.radius);
        builder.appendCircle(cylinderBottom, numPoints);
        Geometry.Circle coneBottom = new Geometry.Circle(puck.center.translateY(-puck.height/2f), top.radius);
